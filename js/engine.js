...
function XMLParser(data) {
   ...
   $(CAT_OBJECT.Categorys).each(function (cat_index, cat_element) {
      ...
      if (Number(cat_element.has_title) === 0) { // ساخت تک صفحه بدون لیست عنوان
         ...
         $(CAT_OBJECT.Articles).each(function (index_art2, art2_element) {
            if (art2_element.cat_id == cat_element.id) {
               $(CAT_OBJECT.Comments).each(function (index_comm2, comm2_element) {
                  if (comm2_element.art_id == art2_element.id) {

                     $(CAT_OBJECT.Answers).each(function (index_answ2, answ2_element) {
   ...
   function getContentFromServer() {
      $.ajax({
         ...
         error: function (err) {
            ...
            console.log("خطا هنگام لود فایل منبع xml: " + err);
         },
      });
   }
